...
def main():
    df_exchange = pd.read_csv('/Users/wago55/Desktop/exchange.csv', encoding = 'cp932', header = 1, 
        names = ['date', 'USD', 'rate'], index_col = 0, parse_dates = True)

    df_jgbcm = pd.read_csv(
        '/Users/wago55/Desktop/jgbcm_all.csv', encoding = 'cp932', header = 1, index_col = 0,
        parse_dates = True, date_parser = parse_japanese_date, na_values = ['-']
    )

    df_jobs = pd.read_excel(
        '/Users/wago55/Desktop/第3表.xlsx', skiprows = 3, skipfooter = 3, usecols = 'W, Y:AJ',
        index_col = 0
    )
    ...
    plt.subplot(3, 1, 2)
    plt.plot(df_jgbcm.index, df_jgbcm['1年'], label = '1年国際金利')
    plt.plot(df_jgbcm.index, df_jgbcm['5年'], label = '5年国際金利')
    plt.plot(df_jgbcm.index, df_jgbcm['10年'], label = '10年国際金利')
    plt.xlim(min_date, max_date)
    plt.legend(loc = 'best')

    plt.subplot(3, 1, 3)
    plt.plot(s_jobs.index, s_jobs, label = '有効求人倍率(季節調整整値)')
    plt.xlim(min_date, max_date)
    plt.ylim(0.0, 2.0)
